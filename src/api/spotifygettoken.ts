import { spotifyReq } from "./initspotify.js";

/* 
verificar se token é valido (acessando getme)
se é:
  continuar com arquivos q importam
...
async function main(){
  const authurl = spotifyReq((api) => api.createAuthorizeURL(['user-read-playback-state', 'user-modify-playback-state', 'user-read-currently-playing',
  ...
  let authres = await spotifyReq((api) => api.authorizationCodeGrant(code))
